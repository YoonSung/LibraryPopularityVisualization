...
Bubbles = function() {
	...
	tick = function(e) {
		...
		node.each(gravity(dampenedAlpha)).each(collide(jitter)).attr(
	...
	force = d3.layout.force().gravity(0).charge(0).size([ width, height ]).on(
			"tick", tick);
	chart = function(selection) {
		return selection.each(function(rawData) {
			...
			node = svgEnter.append("g").attr("id", "bubble-nodes").attr(
					"transform",
					"translate(" + margin.left + "," + margin.top + ")");
			node.append("rect").attr("id", "bubble-background").attr("width",
					width).attr("height", height).on("click", clear);
	...
	updateNodes = function() {
		...
		node.exit().remove();
		return node.enter().append("a").attr("class", "bubble-node").attr(
				"xlink:href", function(d) {
					return "#" + (encodeURIComponent(idValue(d)));
				}).call(force.drag).call(connectEvents).append("circle").attr(
				"r", function(d) {
	...
	updateLabels = function() {
		...
		labelEnter.append("div").attr("class", "bubble-label-name").text(
				function(d) {
					return textValue(d);
				});
		labelEnter.append("div").attr("class", "bubble-label-value").text(
				function(d) {
					// return rValue(d);
					return "★" + d.star + "\n⬇" + d.fork;
				});
		...
		label.append("span").text(function(d) {
			return textValue(d);
				...
				function(d) {
					return d.dx = Math.max(2.5 * rScale(rValue(d)), this
			...
			return data
					.forEach(function(d2) {
						...
						if (d !== d2) {
							...
							minDistance = d.forceR + d2.forceR
									+ collisionPadding;
							if (distance < minDistance) {
								distance = (distance - minDistance) / distance
										* jitter;
								moveX = x * distance;
								moveY = y * distance;
								d.x -= moveX;
